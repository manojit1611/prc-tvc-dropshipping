...
function show_commission_on_admin_order_page($item_id, $item, $product) {
    // Get commission amount per unit
    $commission_amount = $item->get_meta('_commission_amount');
    ...
    if ($commission_amount) {
        ...
        echo '<style>.display_meta { display: none; }</style><p><strong>Commission:</strong> ' . wc_price($commission_amount) . ' × ' . $qty . ' = <strong>' . wc_price($total_commission) . '</strong></p>';
    }
}
...
function my_show_commission_column( $column, $order ) {
    if ( 'order_commission' === $column ) {
        ...
        foreach ( $order->get_items() as $item ) {
            $commission_amount = $item->get_meta('_commission_amount'); // per unit
