...
add_action('rest_api_init', function () {
    register_rest_route('mpi/v1', '/fetch-model', [
        'methods' => 'GET',
        'callback' => 'mpi_fetch_models_callback',
        'permission_callback' => '__return_true', // restrict if needed
    ]);
});

function mpi_fetch_models_callback ()
{
    ...
    $sql = "
        SELECT 
            mpr2.term_id,
            GROUP_CONCAT(DISTINCT mpr2.post_id ORDER BY mpr2.post_id ASC) AS product_ids
        ...
        INNER JOIN {$wpdb->prefix}tvc_manufacturer_product_relation AS mpr2
            ON mpr2.post_id = mpr.post_id 
        AND mpr2.type = " . ww_tvc_get_model_type() . "
        WHERE mpr.type = " . ww_tvc_get_manufacturer_type() . "
        AND mpr2.post_id = " . $current_post_id . "
        AND ( $where_clause )
        GROUP BY mpr2.term_id
