...
class MPI_API {
    public function mpi_get_auth_token()
    {
        ...
        $auth_url = BASE_URL . "/Authorization/GetAuthorization?email=" . urlencode(EMAIL) . "&password=" . urlencode(PASSWORD);
    ...
    public function get_categories_from_api($parentCategoryCode = null)
    {
        ...
        $api_url = BASE_URL . "/OpenApi/Category/GetChildren?ParentCode=" . urlencode($parentCategoryCode);
    ...
    public function get_products_by_category_code($categoryCode)
    {
        $token = $this->mpi_get_auth_token();
        if (!$token) {
            return ['error' => 'Failed to retrieve authentication token'];
        ...
        $api_url = BASE_URL . "/openapi/Product/SearchWithMailCatelog?CategoryCode=" . urlencode($categoryCode);
